...
class TodoController {
    async createTodo(req, res) {
        const { title, description, author, publisher, yearPublished, imageLink } = req.body;
    ...
    async updateTodoById(req, res) {
        const { id } = req.params;
        const { title, description, author, publisher, yearPublished, imageLink, completed } = req.body;

        try {
            const updatedTodo = await Todo.findByIdAndUpdate(
                id,
                { title, description, author, publisher, yearPublished, imageLink, completed },
                { new: true }
            );
            if (!updatedTodo) {
                return res.status(404).json({ message: 'Todo not found' });
